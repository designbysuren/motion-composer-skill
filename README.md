...
A Claude skill for creating production-grade motion graphics, animations, and video compositions using code — GSAP, Remotion, Motion Canvas, ScrollTrigger, SVG, Canvas/WebGL, and AI video APIs.
...
Copy `SKILL.md` into any `.claude/skills/` directory in your project.
...
```
Create a Remotion composition for a 30-second product demo video at 1920×1080.
Use the motion-composer skill.
```
...
```
For any animation, motion graphics, or video composition requests, apply the
motion-composer skill. Reference SKILL.md for the decision tree and patterns.
```

**Codex CLI / Opencode** — add to `AGENTS.md`:
```
## Animation & Motion
For animations, motion graphics, videos, GSAP, Remotion, or ScrollTrigger requests,
apply the rules in SKILL.md (motion-composer skill).
```
